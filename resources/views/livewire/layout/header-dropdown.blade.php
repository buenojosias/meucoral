...
<div class="sm:relative" x-data="{ open: false }" @click.outside="open = false">
    <x-ts-avatar sm :model="auth()->user()" color="fff" class="cursor-pointer" @click="open = !open" />
    <div class="fixed sm:absolute right-0 sm:right-0 top-16 sm:top-12 z-10 w-screen bg-white pr-1.5 sm:rounded-lg drop-shadow-xl dark:bg-slate-700 sm:w-[360px] border"
        ...
        <x-dropdown-link :href="route('panel.profile')" wire:navigate>
            {{ __('Profile') }}
        </x-dropdown-link>
