@if (request()->routeIs('panel*') || request()->routeIs('home'))
    <ul class="my-2 space-y-1">
        ...
        <x-side-link label="Músicas" icon="music-notes" :href="route('panel.songs.index')" :active="request()->routeIs('panel.songs*')" wire:navigate />
...
@elseif (request()->routeIs('admin*'))
    <ul class="mt-4 space-y-1">
        ...
        <x-side-link label="Usuários" icon="users" :href="route('admin.users.index')" :active="request()->routeIs('admin.users*')" wire:navigate />
        <x-side-link label="Corais" icon="users-four" :href="route('admin.choirs.index')" :active="request()->routeIs('admin.choirs*')" wire:navigate />
    </ul>
    <ul class="mt-4 pt-4 space-y-1 border-t border-primary-900">
